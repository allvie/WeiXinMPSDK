...
namespace FGWX.Common
{
    public class Utils
    {
        //public static fastJSON.JSONParameters JSONPARMS = new fastJSON.JSONParameters() { UseEscapedUnicode = false, UseExtensions = false, UseFastGuid = false, UsingGlobalTypes = false, SerializeNullValues = false, UseUTCDateTime = false };
        public static string SafetyMobile(string mobile)
        {
            if (string.IsNullOrWhiteSpace(mobile) || mobile.Length != 11) return "";
            return mobile.Substring(0, 3) + "****" + mobile.Substring(7, 4);
        }
        public static string SafetyEmail(string email)
        {
            if (string.IsNullOrWhiteSpace(email) || email.IndexOf("@") < 1) return "";
            string[] t = email.Split(new string[] { "@" }, StringSplitOptions.RemoveEmptyEntries);
            if (t != null && t.Length == 2)
                if (t[0].Length < 3)
                    return email;
                else
                    return t[0].Substring(0, 1) + "****" + t[0].Substring(t[0].Length - 1, 1) + "@" + t[1];
            else
                return "";
        }

        public static T NullFilter<T>(object obj, T val)
        {
            try
            {
        ...
        public static bool CheckName(string name, int? namefunction, int? minwords, int? maxwords, bool countrepeat)
        {
            bool ok = true;
            if (string.IsNullOrEmpty(name) || (namefunction == 1 && !Regex.IsMatch(name, @"^[\u4e00-\u9fa5]+$", RegexOptions.IgnoreCase)) ||
                (namefunction == 2 && !Regex.IsMatch(name, @"^[A-Za-z ]+$")))
                ok = false;
            else
            {
                if (countrepeat)
                ...
                if (name.Length < minwords || (maxwords > 0 && name.Length > maxwords))
                    ok = false;
            }
            return ok;
        }
        ...
        {
            return StrToDateTime(str, DateTime.UtcNow.AddHours(8));
        }
        ...
        {
            #region
            return DateTime.UtcNow.AddHours(8).ToString("yyyyMMddHHmmssffff");
            #endregion
        }
        #endregion
        ...
        public static string GetCheckCode(int codeCount)
        {
            ...
            int rep = 0;
            long num2 = DateTime.UtcNow.AddHours(8).Ticks + rep;
            rep++;
        ...
        {
            System.Threading.Thread.Sleep(3);
            long tick = DateTime.UtcNow.AddHours(8).Ticks;//1个以0.1纳秒为单位的时间戳，18位
        ...
        {
            string num = DateTime.UtcNow.AddHours(8).ToString("yyMMddHHmmss");//yyyyMMddHHmmssms
        ...
        public static string GetSafeOpenid(string str)
        {
            ...
            if (!IsSafeSqlStringByOpenid(str))
            {
                return "";
            }
            else
            {
                return FilterByOpenid(str);
            }

        }

        public static bool IsSafeSqlStringByOpenid(string str)
        {
            return !Regex.IsMatch(str, @"[;|,|\/|\(|\)|\[|\]|\}|\{|%|@|\*|!|\']");
        }

        public static string FilterByOpenid(string sInput)
        {
            if (sInput == null || sInput == "")
                return null;
            string sInput1 = sInput.ToLower();
            ...
            if (Regex.Match(sInput1, Regex.Escape(pattern), RegexOptions.Compiled | RegexOptions.IgnoreCase).Success)
            {
                throw new Exception("openid字符串中含有非法字符!");
            }
            else
            {
                output = output.Replace("'", "''");
        ...
        public static string GetMapPath(string strPath)
        {
            ...
            else if (strPath.Contains(":/") || strPath.Contains(":\\"))
            {
                return strPath;
            }
        ...
        /// 获得远程字符串
        /// </summary>
        public static string GetDomainStr(string key, string uriPath, int expires = 60)
        {
            ...
            if (result == null)
            {
                ...
                CacheHelper.Insert(key, result, expires);
            }
        ...
        public static void WriteCookie(string strName, string key, string strValue, int expires)
        {
            ...
            cookie[key] = UrlEncode(strValue);
            cookie.Expires = DateTime.UtcNow.AddHours(8).AddMinutes(expires);
            HttpContext.Current.Response.AppendCookie(cookie);
        }
